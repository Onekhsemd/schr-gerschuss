function kartoffelkanone_interaktiv_Version3()
    % Hauptfunktion: Schräger Schuss mit FESTEM Abschussrohr & interaktiver Animation
    % Anders als Version2: Die Länge des Abschussrohres wird fest vorgegeben.
    % Der Druck wird NICHT ideal ausgenutzt – das Projektil verlässt das Rohr
    % sobald es das Ende erreicht, auch wenn noch Überdruck vorhanden ist.
    clear; clc; close all;
    ...
    prompt = {
        ...
        'Länge des Abschussrohres in m (fest vorgegeben):',...
        'Abschusshöhe über dem Boden in m:',...
        'Luftwiderstandsbeiwert (cW):'
    };

    dlgtitle = 'Parameter: Schräger Schuss (Festes Abschussrohr)';
    dims = [1 60];
    definput = {'45', '4', '0.1', '0.05', '0.15', '1.0', '1.5', '0.45'};
    ...
    laenge_rohr  = str2double(answer{6});  % Feste Rohrlänge (Abschussrohr)
    h_start      = str2double(answer{7});
    cw           = str2double(answer{8});
    ...
    % Gesamtlänge bis Rohrmündung: Brennkammer + Abschussrohr
    laenge_gesamt = s_start + laenge_rohr;

    %% 3. Phase: Beschleunigung im Abschussrohr (endet an fester Rohrlänge)
    i = 1; t(1) = 0; dt = 0.0001;
    s(1) = s_start; v(1) = 0; V(1) = A_r * s(1);
    a(1) = ((P_innen(1) - P_ausen) * A_k) / m_k - g * sin(phi);

    % Schleife läuft bis das Projektil die Rohrmündung erreicht
    % (nicht bis der Druck ausgeglichen ist → Druck wird nicht ideal ausgenutzt)
    while s(i) < laenge_gesamt
        i = i + 1;
        t(i) = t(i-1) + dt;
        ...
        % Sicherheitsabbruch: falls Geschwindigkeit negativ wird
        if v(i) < 0
            v(i) = 0;
            break;
        end
    ...
    % Informationen zum Druckausnutzungsgrad ausgeben
    P_muendung = P_innen(end);
    ausnutzung = (1 - (P_muendung - P_ausen) / (P_innen(1) - P_ausen)) * 100;
    fprintf('--- Ergebnis Beschleunigungsphase ---\n');
    fprintf('Abgangsgeschwindigkeit:   %.2f m/s\n', v(end));
    fprintf('Druck an der Mündung:     %.2f bar (absolut)\n', P_muendung / 100000);
    fprintf('Startdruck:               %.2f bar (absolut)\n', P_innen(1) / 100000);
    fprintf('Druckausnutzungsgrad:     %.1f %%\n', ausnutzung);
    if P_muendung > P_ausen * 1.01  % 1% Schwellwert für signifikanten Restdruck
        fprintf('HINWEIS: An der Mündung ist noch Überdruck (%.2f bar) vorhanden –\n', (P_muendung - P_ausen) / 100000);
        fprintf('         der Druck wurde NICHT vollständig ausgenutzt.\n');
    else
        fprintf('HINWEIS: Der Druck wurde nahezu vollständig ausgenutzt.\n');
        fprintf('         Für diesen Startdruck wäre ein kürzeres Rohr möglich.\n');
    end
    fprintf('--------------------------------------\n');
    ...
    s_ver(1) = h_start + laenge_gesamt * sin(phi);
    s_hor(1) = laenge_gesamt * cos(phi);
    ...
    % Titelinformation mit Druckausnutzung
    titel_info = sprintf('Festes Rohr: %.2f m | Abgangsgeschwindigkeit: %.1f m/s | Druck an Muendung: %.2f bar | Ausnutzung: %.1f%%', ...
        laenge_rohr, v(end), P_muendung/100000, ausnutzung);

    % Großes Hauptfenster erstellen
    fig = figure('Name', ['Interaktives Dashboard: Festes Abschussrohr – ' titel_info], ...
    ...
    function speedCallback(src, ~)
        playback_speed = speed_values(src.Value);
    end
    ...
    function playPauseCallback(src, ~)
        ...
        if isPlaying
            ...
            src.String = 'PLAY';
            src.ForegroundColor = [0 0.5 0];
        end
    end

    function sliderCallback(src, ~)
        current_idx = round(src.Value);
        virtual_idx = current_idx;
        updatePlots();
    end
